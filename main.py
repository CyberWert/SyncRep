...
"""
    class Sqwork:
        ...
        def compare(con):
            ...
            c)	Получить все адреса магазинов с названием, начинающихся на английскую букву “I”. Использовать LIKE (см. файл “Дополнительно).
            d)	Получить все названия товаров, которые продаются в отделах с мебелью (sphere == Furniture).
            e)	Получить названия магазинов, где в продаже есть товары с описанием.
            Для сбора только уникальных значений пишем в самом начале SELECT DISTINCT.
            f)	Получить для каждого товара все его поля (кроме id) + все поля его отдела (кроме id), причем для всех полей отдела в ответе должна быть приписка
             department_{название_поля} + все поля его магазина (кроме id) с припиской shop_{название_поля}.
            """
            curs = con.cursor()
